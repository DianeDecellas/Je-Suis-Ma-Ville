...
public class XmlMenu : MonoBehaviour
{
    ...
    void readXmlMenu(XmlNode menu)
    {
         ...
         * g.transform.GetChild(0).GetComponent<Text>().text = "Balade Démo MVP";
        g.transform.GetChild(1).GetComponent<Text>().text = "00h30";
        void loadBalade0()
        {
            UrlStorage.url = "www.demoMVP.fr";
            Debug.Log("www.demoMVP.fr");
            SceneManager.LoadScene("scene_finale");
        }
        g.GetComponent<Button>().onClick.AddListener(loadBalade0);
        */
